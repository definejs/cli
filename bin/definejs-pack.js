...
if (!config) {
    console.log('make sure the current directory has the `config.js` file, or the config object is valid.'.red)
    return;
}

pack(config, () => {
    //输出最终的 config 以供查阅。
    File.writeJSON(`${config.outputDir}config-lock.json`, config);
});
